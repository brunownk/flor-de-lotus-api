...
public interface AnimalBreedRepository extends JpaRepository<AnimalBreed, Long> {
    ...
    @Query("SELECT a FROM AnimalBreed a WHERE a.deletedAt IS NULL AND LOWER(a.name) LIKE LOWER(CONCAT('%', :name, '%'))")
    Page<AnimalBreed> searchActiveByName(@Param("name") String name, Pageable pageable);

    @Query("SELECT a FROM AnimalBreed a WHERE LOWER(a.name) LIKE LOWER(CONCAT('%', :name, '%'))")
